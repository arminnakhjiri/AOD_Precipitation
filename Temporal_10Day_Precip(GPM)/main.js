...
var startDate = ee.Date('2023-01-01'); // Start date
...
function createCombinedImage(dateMillis) {
  ...
  var precipitation_avg = calculateIntervalAverage(GPM, 'precipitation', date); // Calculate precipitation average
  
  var combinedImage = precipitation_avg.set('system:time_start', date); // Set timestamp
  return combinedImage;
}

// Load GPM IMERG V07 ImageCollection for precipitation data
...
var chart = ui.Chart.image.series(
  ...
  11132, // GPM resolution in meters
...
function imageToFeature(image) {
  ...
  var stats = image.reduceRegion({
    ...
    scale: 11132, // Resolution in meters
...
Export.table.toDrive({
  ...
  description: 'GPM_Precipitation_238', // File description
  folder: 'GEE_Exports', // Destination folder
  fileNamePrefix: 'GPM_Precipitation_238', // File name prefix
